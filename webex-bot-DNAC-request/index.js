// Created and Edited by Yunxia and Yecheng
...
*/
framework.hears(/helpme|what can i (do|say)|what (can|do) you do/i, function (bot, trigger) {
...
framework.hears('GetToken', function (bot) {
  console.log(`Getting token from DNAC ...`);
  responded = true;

  request(tokenHeader, function (error, response, body) {
    if (error){
      console.log(error);
    };
    tmp.token = body.slice(10,-2);
    //console.log(tmp.token);
  });
  bot.say("markdown",'Token got!');
...
framework.hears('NetworkDevice', function (bot) {
  console.log(`Getting network devices information from DNAC ...`);
  responded = true;
  if(!tmp.token){
    console.log('Token expired!');
    bot.say("markdown",'Token expired!')
  }else{
    netDeviceHeader['headers']['X-Auth-Token'] = tmp.token;
    request(netDeviceHeader, function (error, response, body) {
      if (error){
        console.log(error);
      };
      ...
      //bot.say("markdown",'This is device information' + JSON.stringify(raw));
      for(var i=0; i<raw.length; i++){
        console.log(i + JSON.stringify(raw[i]));
        let st = [];
        let rawtemp = JSON.stringify(raw[i]).split(",");
        for(var j=0;j<rawtemp.length;j++){
          st = st + rawtemp[j] + '\n';
        }
        bot.say('markdown', '************* Device Information **************' + '\n' + st + '\n');
...
function sendHelp(bot) {
  bot.say("markdown", 'These are the commands I can respond to:', '\n\n ' +
    ...
    '7. **helpme** (what you are reading now) \n' +
    '8. **GetToken** (Get token from DNAC)\n' +
    '9. **NetworkDevice** (Get network devices informaiton from DNAC)');
}
