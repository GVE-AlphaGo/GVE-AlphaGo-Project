...
import { CryptoKey as CryptoKey$1, AlgorithmError, OperationError, AesCbcProvider as AesCbcProvider$1, AesCmacProvider as AesCmacProvider$1, AesCtrProvider as AesCtrProvider$1, AesGcmProvider as AesGcmProvider$1, AesKwProvider as AesKwProvider$1, AesEcbProvider as AesEcbProvider$1, DesProvider, CryptoError, RsaSsaProvider as RsaSsaProvider$1, RsaPssProvider as RsaPssProvider$1, RsaOaepProvider as RsaOaepProvider$1, BufferSourceConverter, ProviderCrypto, EcdsaProvider as EcdsaProvider$1, EcdhProvider as EcdhProvider$1, Pbkdf2Provider as Pbkdf2Provider$1, HmacProvider as HmacProvider$1, HkdfProvider as HkdfProvider$1, SubtleCrypto as SubtleCrypto$1 } from 'webcrypto-core';
import crypto__default, { createCipheriv, publicEncrypt, privateDecrypt, constants } from 'crypto';
import { Convert } from 'pvtsutils';
import { Integer } from 'asn1js';
import { AsnProp, AsnPropTypes, AsnType, AsnTypeTypes, AsnIntegerConverter, AsnSerializer, AsnParser } from '@peculiar/asn1-schema';
...
class RsaPrivateKey$1 extends AsymmetricKey {
    ...
    getKey() {
        const keyInfo = AsnParser.parse(this.data, PrivateKeyInfo);
        return AsnParser.parse(keyInfo.privateKey, RsaPrivateKey);
    }
    toJSON() {
    ...
    fromJSON(json) {
        const key = JsonParser.fromJSON(json, { targetSchema: RsaPrivateKey });
...
class RsaPublicKey$1 extends AsymmetricKey {
    ...
    getKey() {
        const keyInfo = AsnParser.parse(this.data, PublicKeyInfo);
        return AsnParser.parse(keyInfo.publicKey, RsaPublicKey);
    }
    toJSON() {
        const key = this.getKey();
        const json = {
    ...
    fromJSON(json) {
        const key = JsonParser.fromJSON(json, { targetSchema: RsaPublicKey });
    ...
    static async importKey(format, keyData, algorithm, extractable, keyUsages) {
        switch (format.toLowerCase()) {
            case "jwk":
                const jwk = keyData;
                if (jwk.d) {
                    const asnKey = JsonParser.fromJSON(keyData, { targetSchema: RsaPrivateKey });
                    return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
                }
                else {
                    const asnKey = JsonParser.fromJSON(keyData, { targetSchema: RsaPublicKey });
                    return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
                }
            case "spki": {
                const keyInfo = AsnParser.parse(new Uint8Array(keyData), PublicKeyInfo);
                const asnKey = AsnParser.parse(keyInfo.publicKey, RsaPublicKey);
                return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
            }
            case "pkcs8": {
                const keyInfo = AsnParser.parse(new Uint8Array(keyData), PrivateKeyInfo);
                const asnKey = AsnParser.parse(keyInfo.privateKey, RsaPrivateKey);
                return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
            }
            default:
                throw new OperationError("format: Must be 'jwk', 'pkcs8' or 'spki'");
...
class EcPrivateKey$1 extends AsymmetricKey {
    ...
    getKey() {
        const keyInfo = AsnParser.parse(this.data, PrivateKeyInfo);
        return AsnParser.parse(keyInfo.privateKey, EcPrivateKey);
    }
    toJSON() {
        const key = this.getKey();
        const json = {
    ...
    fromJSON(json) {
        if (!json.alg) {
            throw new OperationError(`Cannot get named curve from JWK. Property 'alg' is required`);
        }
        const keyInfo = new PrivateKeyInfo();
        keyInfo.privateKeyAlgorithm.algorithm = "1.2.840.10045.2.1";
        keyInfo.privateKeyAlgorithm.parameters = AsnSerializer.serialize(new ObjectIdentifier(getOidByNamedCurve(json.crv)));
        const key = JsonParser.fromJSON(json, { targetSchema: EcPrivateKey });
        keyInfo.privateKey = AsnSerializer.serialize(key);
        this.data = Buffer.from(AsnSerializer.serialize(keyInfo));
        return this;
    }
...
class EcPublicKey$1 extends AsymmetricKey {
    ...
    fromJSON(json) {
        const key = JsonParser.fromJSON(json, { targetSchema: EcPublicKey });
        const keyInfo = new PublicKeyInfo();
        keyInfo.publicKeyAlgorithm.algorithm = "1.2.840.10045.2.1";
        keyInfo.publicKeyAlgorithm.parameters = AsnSerializer.serialize(new ObjectIdentifier(getOidByNamedCurve(json.crv)));
...
class EcCrypto {
    ...
    static async sign(algorithm, key, data) {
        ...
        const signature = signer.sign(options);
        const ecSignature = AsnParser.parse(signature, EcDsaSignature);
    ...
    static async deriveBits(algorithm, baseKey, length) {
        ...
        const asnPrivateKey = AsnParser.parse(baseKey.data, PrivateKeyInfo);
        const asnEcPrivateKey = AsnParser.parse(asnPrivateKey.privateKey, EcPrivateKey);
        ecdh.setPrivateKey(Buffer.from(asnEcPrivateKey.privateKey));
        const asnPublicKey = AsnParser.parse(algorithm.public.data, PublicKeyInfo);
    ...
    static async exportKey(format, key) {
        switch (format.toLowerCase()) {
            ...
            case "spki":
                return new Uint8Array(key.data).buffer;
            case "raw":
                const publicKeyInfo = AsnParser.parse(key.data, PublicKeyInfo);
    ...
    static async importKey(format, keyData, algorithm, extractable, keyUsages) {
        switch (format.toLowerCase()) {
            case "jwk":
                const jwk = keyData;
                if (jwk.d) {
                    const asnKey = JsonParser.fromJSON(keyData, { targetSchema: EcPrivateKey });
                    return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
                }
                else {
                    const asnKey = JsonParser.fromJSON(keyData, { targetSchema: EcPublicKey });
                    return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
                }
            case "raw": {
            ...
            case "pkcs8": {
                const keyInfo = AsnParser.parse(new Uint8Array(keyData), PrivateKeyInfo);
                const asnKey = AsnParser.parse(keyInfo.privateKey, EcPrivateKey);
                return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
            }
            default:
                throw new OperationError("format: Must be 'jwk', 'raw', 'pkcs8' or 'spki'");
