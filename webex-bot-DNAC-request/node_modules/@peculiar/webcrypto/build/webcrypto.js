...
class RsaPrivateKey$1 extends AsymmetricKey {
    constructor() {
        super(...arguments);
        this.type = "private";
    }
    getKey() {
        const keyInfo = asn1Schema.AsnParser.parse(this.data, PrivateKeyInfo);
        return asn1Schema.AsnParser.parse(keyInfo.privateKey, RsaPrivateKey);
    }
    toJSON() {
    ...
    fromJSON(json) {
        const key = jsonSchema.JsonParser.fromJSON(json, { targetSchema: RsaPrivateKey });
...
class RsaPublicKey$1 extends AsymmetricKey {
    constructor() {
        super(...arguments);
        this.type = "public";
    }
    getKey() {
        const keyInfo = asn1Schema.AsnParser.parse(this.data, PublicKeyInfo);
        return asn1Schema.AsnParser.parse(keyInfo.publicKey, RsaPublicKey);
    }
    toJSON() {
        const key = this.getKey();
        const json = {
    ...
    fromJSON(json) {
        const key = jsonSchema.JsonParser.fromJSON(json, { targetSchema: RsaPublicKey });
    ...
    static async importKey(format, keyData, algorithm, extractable, keyUsages) {
        switch (format.toLowerCase()) {
            case "jwk":
                const jwk = keyData;
                if (jwk.d) {
                    const asnKey = jsonSchema.JsonParser.fromJSON(keyData, { targetSchema: RsaPrivateKey });
                    return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
                }
                else {
                    const asnKey = jsonSchema.JsonParser.fromJSON(keyData, { targetSchema: RsaPublicKey });
                    return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
                }
            case "spki": {
                const keyInfo = asn1Schema.AsnParser.parse(new Uint8Array(keyData), PublicKeyInfo);
                const asnKey = asn1Schema.AsnParser.parse(keyInfo.publicKey, RsaPublicKey);
                return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
            }
            case "pkcs8": {
                const keyInfo = asn1Schema.AsnParser.parse(new Uint8Array(keyData), PrivateKeyInfo);
                const asnKey = asn1Schema.AsnParser.parse(keyInfo.privateKey, RsaPrivateKey);
                return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
            }
            default:
                throw new core.OperationError("format: Must be 'jwk', 'pkcs8' or 'spki'");
...
class RsaSsaProvider extends core.RsaSsaProvider {
    ...
    checkCryptoKey(key, keyUsage) {
        super.checkCryptoKey(key, keyUsage);
        if (!(key instanceof RsaPrivateKey$1 || key instanceof RsaPublicKey$1)) {
...
class RsaOaepProvider extends core.RsaOaepProvider {
    ...
    async onEncrypt(algorithm, key, data) {
        ...
        const labelHash = crypto__default.createHash(key.algorithm.hash.name.replace("-", ""))
...
class EcPrivateKey$1 extends AsymmetricKey {
    constructor() {
        super(...arguments);
        this.type = "private";
    }
    getKey() {
        const keyInfo = asn1Schema.AsnParser.parse(this.data, PrivateKeyInfo);
        return asn1Schema.AsnParser.parse(keyInfo.privateKey, EcPrivateKey);
    }
    toJSON() {
        const key = this.getKey();
        const json = {
    ...
    fromJSON(json) {
        ...
        keyInfo.privateKeyAlgorithm.parameters = asn1Schema.AsnSerializer.serialize(new ObjectIdentifier(getOidByNamedCurve(json.crv)));
        const key = jsonSchema.JsonParser.fromJSON(json, { targetSchema: EcPrivateKey });
        keyInfo.privateKey = asn1Schema.AsnSerializer.serialize(key);
        this.data = Buffer.from(asn1Schema.AsnSerializer.serialize(keyInfo));
        return this;
    }
...
class EcPublicKey$1 extends AsymmetricKey {
    constructor() {
        super(...arguments);
        this.type = "public";
    }
    getKey() {
        const keyInfo = asn1Schema.AsnParser.parse(this.data, PublicKeyInfo);
        return new EcPublicKey(keyInfo.publicKey);
    }
    toJSON() {
        const key = this.getKey();
        const json = {
    ...
    fromJSON(json) {
        const key = jsonSchema.JsonParser.fromJSON(json, { targetSchema: EcPublicKey });
        const keyInfo = new PublicKeyInfo();
        keyInfo.publicKeyAlgorithm.algorithm = "1.2.840.10045.2.1";
        keyInfo.publicKeyAlgorithm.parameters = asn1Schema.AsnSerializer.serialize(new ObjectIdentifier(getOidByNamedCurve(json.crv)));
...
class EcCrypto {
    ...
    static async deriveBits(algorithm, baseKey, length) {
        ...
        const asnPrivateKey = asn1Schema.AsnParser.parse(baseKey.data, PrivateKeyInfo);
        const asnEcPrivateKey = asn1Schema.AsnParser.parse(asnPrivateKey.privateKey, EcPrivateKey);
        ecdh.setPrivateKey(Buffer.from(asnEcPrivateKey.privateKey));
        const asnPublicKey = asn1Schema.AsnParser.parse(algorithm.public.data, PublicKeyInfo);
    ...
    static async exportKey(format, key) {
        switch (format.toLowerCase()) {
            ...
            case "spki":
                return new Uint8Array(key.data).buffer;
            case "raw":
                const publicKeyInfo = asn1Schema.AsnParser.parse(key.data, PublicKeyInfo);
    ...
    static async importKey(format, keyData, algorithm, extractable, keyUsages) {
        switch (format.toLowerCase()) {
            case "jwk":
                const jwk = keyData;
                if (jwk.d) {
                    const asnKey = jsonSchema.JsonParser.fromJSON(keyData, { targetSchema: EcPrivateKey });
                    return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
                }
                else {
                    const asnKey = jsonSchema.JsonParser.fromJSON(keyData, { targetSchema: EcPublicKey });
                    return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
                }
            case "raw": {
                const asnKey = new EcPublicKey(keyData);
                return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
            }
            case "spki": {
                const keyInfo = asn1Schema.AsnParser.parse(new Uint8Array(keyData), PublicKeyInfo);
                const asnKey = new EcPublicKey(keyInfo.publicKey);
                return this.importPublicKey(asnKey, algorithm, extractable, keyUsages);
            }
            case "pkcs8": {
                const keyInfo = asn1Schema.AsnParser.parse(new Uint8Array(keyData), PrivateKeyInfo);
                const asnKey = asn1Schema.AsnParser.parse(keyInfo.privateKey, EcPrivateKey);
                return this.importPrivateKey(asnKey, algorithm, extractable, keyUsages);
            }
            default:
                throw new core.OperationError("format: Must be 'jwk', 'raw', 'pkcs8' or 'spki'");
    ...
    static async importPrivateKey(asnKey, algorithm, extractable, keyUsages) {
        const keyInfo = new PrivateKeyInfo();
        keyInfo.privateKeyAlgorithm.algorithm = "1.2.840.10045.2.1";
        keyInfo.privateKeyAlgorithm.parameters = asn1Schema.AsnSerializer.serialize(new ObjectIdentifier(getOidByNamedCurve(algorithm.namedCurve)));
        keyInfo.privateKey = asn1Schema.AsnSerializer.serialize(asnKey);
        const key = new EcPrivateKey$1();
        key.data = Buffer.from(asn1Schema.AsnSerializer.serialize(keyInfo));
        key.algorithm = Object.assign({}, algorithm);
        key.extractable = extractable;
        key.usages = keyUsages;
    ...
    static async importPublicKey(asnKey, algorithm, extractable, keyUsages) {
        const keyInfo = new PublicKeyInfo();
        keyInfo.publicKeyAlgorithm.algorithm = "1.2.840.10045.2.1";
        keyInfo.publicKeyAlgorithm.parameters = asn1Schema.AsnSerializer.serialize(new ObjectIdentifier(getOidByNamedCurve(algorithm.namedCurve)));
        keyInfo.publicKey = asnKey.value;
        const key = new EcPublicKey$1();
        key.data = Buffer.from(asn1Schema.AsnSerializer.serialize(keyInfo));
        key.algorithm = Object.assign({}, algorithm);
        key.extractable = extractable;
        key.usages = keyUsages;
...
class EcdsaProvider extends core.EcdsaProvider {
    ...
    checkCryptoKey(key, keyUsage) {
        super.checkCryptoKey(key, keyUsage);
        if (!(key instanceof EcPrivateKey$1 || key instanceof EcPublicKey$1)) {
...
class Pbkdf2Provider extends core.Pbkdf2Provider {
    async onDeriveBits(algorithm, baseKey, length) {
        return new Promise((resolve, reject) => {
            ...
            crypto__default.pbkdf2(baseKey.data, Buffer.from(salt), algorithm.iterations, length >> 3, hash, (err, derivedBits) => {
                if (err) {
                    reject(err);
                }
                else {
...
class HmacProvider extends core.HmacProvider {
    ...
    async onSign(algorithm, key, data) {
        ...
        const hmac = crypto__default.createHmac(hash, key.data)
            .update(Buffer.from(data)).digest();
...
class HkdfProvider extends core.HkdfProvider {
    ...
    async onDeriveBits(params, baseKey, length) {
        ...
        const PRK = crypto__default.createHmac(hash, core.BufferSourceConverter.toUint8Array(params.salt))
            .update(core.BufferSourceConverter.toUint8Array(baseKey.data))
            .digest();
